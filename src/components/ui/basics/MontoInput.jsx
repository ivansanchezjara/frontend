 ...
 * - label: string (opcional) — etiqueta superior
 * - error: string (opcional) — mensaje de error
 * - disabled: boolean (opcional)
 * - className: string (opcional) — clases extras para el contenedor
 * - ...rest: se pasa al input
 */
export default function MontoInput({ value, onChange, moneda = "PYG", label, error, disabled, className = "", ...rest }) {
    ...
    <div className={className}>
      ...
      <div className="relative">
        ...
        <input
          ...
          onBlur={handleBlur}
          disabled={disabled}
          className={`w-full rounded-lg border pl-10 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-right font-mono transition-all ${
            disabled ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-slate-50 focus:bg-white"
