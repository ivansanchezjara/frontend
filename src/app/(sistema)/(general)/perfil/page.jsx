...
export default function ProfilePage() {
  ...
  // Cargar perfil con useApi
  const { loading, execute: refreshProfile } = useApi(getProfile, {
    auto: true,
    initialData: null,
    ...
    <div className="p-4 md:p-10 max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col items-center text-center">
            ...
            <div className="mt-8 w-full border-t border-slate-50 pt-8 space-y-4">
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="bg-emerald-50 text-emerald-600 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest">
                  Activo
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {profile?.groups?.length > 0 ? (
                profile.groups.map((group, idx) => (
                  <span
                    key={idx}
                    className="bg-white/10 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg border border-white/10 uppercase tracking-widest"
                  >
                    {group}
                  </span>
                ))
        ...
        <div className="md:col-span-2">
          <form
            onSubmit={handleSave}
            className="bg-white p-8 md:p-10 rounded-[40px] border border-slate-200 shadow-sm space-y-8"
          >
