...
export default function NuevoIngresoPage() {
    ...
    const [ingreso, setIngreso] = useState({
        ...
        descripcion: 'Ingreso mensual de mercadería',
        comprobante: '',
        deposito: ''
    });
    ...
    const validateItems = (currentItems) => {
        ...
        for (let i = 0; i < currentItems.length; i++) {
            ...
            if (parseFloat(it.cantidad) <= 0) error = `Ítem ${i + 1}: Cantidad debe ser mayor a 0.`;
            else if (!it.lote_codigo || it.lote_codigo.trim() === "") error = `Ítem ${i + 1}: El código de lote es obligatorio (use S/L si no tiene).`;
            else if (fob < 0 || landed < 0 || p0 < 0 || p1 < 0 || p2 < 0 || p3 < 0 || p4 < 0) error = `Ítem ${i + 1}: No se permiten valores negativos.`;
            else if (fob > landed) error = `Ítem ${i + 1}: Costo FOB no puede superar al Landed.`;
    ...
    const handleDownloadTemplate = () => {
        ...
        link.setAttribute("download", `plantilla_ingreso_${new Date().getTime()}.csv`);
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                    <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-20">
                        ...
                        <div className="flex gap-2">
                            <input type="file" id="csvImport" className="hidden" accept=".csv" onChange={(e) => {
                            ...
                            <button type="button" onClick={() => setIsSearchOpen(true)} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl text-xs font-black flex items-center gap-2 shadow-lg shadow-blue-100 transition-all active:scale-95">
