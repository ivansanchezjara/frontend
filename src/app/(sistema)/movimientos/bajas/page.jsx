...
import { Trash2, AlertTriangle, Eye, ChevronRight, Package, User, Calendar, Plus, Clock } from 'lucide-react';
    ...
    return (
        <div className="p-4 md:p-10 max-w-7xl mx-auto space-y-6 bg-slate-50 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight text-rose-600">Bajas de Inventario</h1>
                    <div className="flex items-center gap-2 text-slate-500 text-sm mt-1">
                        <Link href="/movimientos" className="hover:text-rose-600 transition-colors">Movimientos</Link>
                        <ChevronRight size={14} />
                        <span className="font-bold text-slate-400 uppercase tracking-widest text-[10px]">Mermas y Pérdidas</span>
                    </div>
                </div>
                <Link
                    href="/movimientos/bajas/nuevo"
                    className="bg-rose-600 hover:bg-rose-700 text-white font-black py-4 px-8 rounded-[24px] shadow-xl shadow-rose-100 transition-all flex items-center gap-2 uppercase text-xs tracking-widest"
                >
                    <Plus size={18} /> Registrar Nueva Baja
                </Link>
            </div>
