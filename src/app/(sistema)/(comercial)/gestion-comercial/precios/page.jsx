...
/** Convierte un ISO datetime (UTC) a string "YYYY-MM-DDTHH:mm" en horario Paraguay */
...
/** Convierte un "YYYY-MM-DDTHH:mm" en horario Paraguay a ISO UTC para enviar al backend */
function toUTCFromLocal(localString) {
  // Construimos la fecha interpretándola como America/Asuncion
  // Paraguay es UTC-4 (estándar) / UTC-3 (verano), usamos el approach de calcular offset
  const date = new Date(localString);
  // Crear un formatter que nos de el offset real de Paraguay para esa fecha
  const utcDate = new Date(date.toLocaleString('en-US', { timeZone: 'UTC' }));
  const pyDate = new Date(date.toLocaleString('en-US', { timeZone: TZ }));
  const offset = utcDate - pyDate; // offset en ms (positivo = Paraguay está atrás de UTC)
  // La fecha ingresada es en horario PY, así que sumamos el offset para obtener UTC
...
export default function PreciosPage() {
  ...
  const startEditing = (variante) => {
    ...
    vals.oferta_inicio = variante.oferta_inicio ? toLocalDatetime(variante.oferta_inicio) : '';
    vals.oferta_vence = variante.oferta_vence ? toLocalDatetime(variante.oferta_vence) : '';
    vals.tasa_iva = variante.tasa_iva;
    setEditValues(vals);
    setInitialEditValues(vals);
  };
  ...
  const hasChanges = () => {
    return Object.keys(editValues).some(
      (key) => String(editValues[key] ?? '') !== String(initialEditValues[key] ?? '')
    );
  };
  ...
  const saveRow = async () => {
    try {
      ...
      if (editValues.oferta_inicio) {
        payload.oferta_inicio = toUTCFromLocal(editValues.oferta_inicio);
      } else {
        payload.oferta_inicio = null;
      }
      if (editValues.oferta_vence) {
        payload.oferta_vence = toUTCFromLocal(editValues.oferta_vence);
      } else {
  ...
  const handleBulkUpdate = async (bulkData) => {
    try {
      ...
      showToast(`${res.actualizadas} variantes actualizadas`, 'success');
      setSelected([]);
            ...
            <>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div>
                  ...
                  <Heading level={6} className="text-slate-800">{editingRow.producto_nombre}</Heading>
                  <Text variant="bodyXs" className="text-slate-500 font-medium">{editingRow.nombre_variante}</Text>
                </div>
              ...
              <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-1">
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Input type="number" step="0.01" label="Costo FOB (USD)" value={editValues.costo_fob ?? ''} onChange={(e) => setEditValues({ ...editValues, costo_fob: e.target.value })} placeholder="0.00" />
                    <Input type="number" step="0.01" label="Costo Landed (USD)" value={editValues.costo_landed ?? ''} onChange={(e) => setEditValues({ ...editValues, costo_landed: e.target.value })} placeholder="0.00" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <Input type="number" step="0.01" label="Público" value={editValues.precio_0_publico ?? ''} onChange={(e) => setEditValues({ ...editValues, precio_0_publico: e.target.value })} placeholder="0.00" />
                    <Input type="number" step="0.01" label="Estudiante" value={editValues.precio_1_estudiante ?? ''} onChange={(e) => setEditValues({ ...editValues, precio_1_estudiante: e.target.value })} placeholder="0.00" />
                    <Input type="number" step="0.01" label="Reventa" value={editValues.precio_2_reventa ?? ''} onChange={(e) => setEditValues({ ...editValues, precio_2_reventa: e.target.value })} placeholder="0.00" />
                    <Input type="number" step="0.01" label="Mayorista" value={editValues.precio_3_mayorista ?? ''} onChange={(e) => setEditValues({ ...editValues, precio_3_mayorista: e.target.value })} placeholder="0.00" />
                    <Input type="number" step="0.01" label="Intercompany" value={editValues.precio_4_intercompany ?? ''} onChange={(e) => setEditValues({ ...editValues, precio_4_intercompany: e.target.value })} placeholder="0.00" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    <Input type="number" step="0.01" label="Precio Oferta" value={editValues.precio_oferta ?? ''} onChange={(e) => setEditValues({ ...editValues, precio_oferta: e.target.value })} placeholder="0.00" />
                    <Input type="datetime-local" label="Inicio Oferta" value={editValues.oferta_inicio ?? ''} onChange={(e) => setEditValues({ ...editValues, oferta_inicio: e.target.value })} />
                    <Input type="datetime-local" label="Vencimiento Oferta" value={editValues.oferta_vence ?? ''} onChange={(e) => setEditValues({ ...editValues, oferta_vence: e.target.value })} />
                    <div className="flex flex-col gap-1.5 w-full">
                      <Text as="label" variant="label" className="text-xs font-semibold text-slate-700">Tasa IVA</Text>
                      <select
              ...
              <div className="flex justify-end items-center gap-3 pt-4 border-t border-slate-100">
                <Button variant="ghost" onClick={cancelEditing} disabled={saving}>Cancelar</Button>
                <Button onClick={saveRow} disabled={saving || !hasChanges()}>
