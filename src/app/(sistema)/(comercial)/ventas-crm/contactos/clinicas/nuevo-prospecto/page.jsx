...
import {
  Loader2, Building2, Check, Upload, FileSpreadsheet, AlertCircle, X, Download,
} from "lucide-react";
...
const TABS = [
  { id: "unitario", label: "Uno a uno", icon: Building2 },
...
function TabUnitario({ recientes, setRecientes }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.contacto_compras_nombre.trim()) payload.contacto_compras_nombre = formData.contacto_compras_nombre.trim();
      if (formData.contacto_compras_telefono.trim()) payload.contacto_compras_telefono = formData.contacto_compras_telefono.trim();
      ...
      setRecientes((prev) => [
        { id: nueva.id, nombre: nueva.razon_social, telefono: nueva.telefono },
        ...prev,
      ]);
      setFormData({
        razon_social: "", telefono: "", contacto_compras_nombre: "",
        contacto_compras_telefono: "", departamento: "", ciudad: "", direccion: "",
      });
      showToast(`Clínica "${nueva.razon_social}" creada como prospecto`, "success");
  ...
  return (
    <Section
      title="Registrar Prospecto — Clínica"
      subtitle="Consultorios y clínicas dentales potenciales. Enter para guardar y seguir."
    >
      <div className="p-6 space-y-4" onKeyDown={handleKeyDown}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            ...
            value={formData.contacto_compras_nombre}
          ...
          <Input
            ...
            value={formData.contacto_compras_telefono}
...
function parseCSV(text) {
  ...
  const KNOWN_HEADERS = [
    "nombre", "razon_social", "clinica",
    "telefono", "celular", "phone",
    "contacto", "contacto_compras", "encargado",
    "tel_contacto", "tel_compras",
    "departamento", "ciudad", "direccion",
  ];
  const hasHeader = header.some((h) => KNOWN_HEADERS.includes(h));
  const dataRows = hasHeader ? rows.slice(1) : rows;

  let nameCol = 0, phoneCol = 1, contactCol = -1, contactPhoneCol = -1;
  let depCol = -1, cityCol = -1, dirCol = -1;

  if (hasHeader) {
    const find = (keywords) => header.findIndex((h) => keywords.includes(h));
    const nameIdx = find(["nombre", "razon_social", "clinica"]);
    const phoneIdx = find(["telefono", "celular", "phone"]);
    contactCol = find(["contacto", "contacto_compras", "encargado"]);
    contactPhoneCol = find(["tel_contacto", "tel_compras"]);
    depCol = find(["departamento"]);
    cityCol = find(["ciudad"]);
    dirCol = find(["direccion"]);
  ...
  return dataRows
    .map((row, idx) => {
      ...
      if (contactCol >= 0 && row[contactCol]) item.contacto_compras_nombre = row[contactCol];
      if (contactPhoneCol >= 0 && row[contactPhoneCol]) item.contacto_compras_telefono = row[contactPhoneCol];
      if (depCol >= 0 && row[depCol]) item.departamento = row[depCol];
      if (cityCol >= 0 && row[cityCol]) item.ciudad = row[cityCol];
      if (dirCol >= 0 && row[dirCol]) item.direccion = row[dirCol];
...
function TabMasivo({ recientes, setRecientes }) {
  ...
  const handleDownloadTemplate = () => {
    const headers = [
      ...
      "Contacto Compras (opcional)",
      "Tel Contacto (opcional)",
      "Departamento (opcional)",
      "Ciudad (opcional)",
      "Direccion (opcional)",
    ];
    const example1 = "OdontoCenter SRL;021555444;Lic. María López;0981123456;Central;Asunción;Av. España 1234";
    const example2 = "Dental Plus;021666555;Carlos Ruiz;0971222333;Central;San Lorenzo;";
    const csvContent = [headers.join(";"), example1, example2].join("\n");
    const blob = new Blob(["\uFEFF" + csvContent], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", "plantilla_prospectos_clinicas.csv");
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < preview.length; i++) {
      ...
      try {
        ...
        if (item.contacto_compras_nombre) payload.contacto_compras_nombre = item.contacto_compras_nombre;
        if (item.contacto_compras_telefono) payload.contacto_compras_telefono = item.contacto_compras_telefono;
        if (item.departamento) payload.departamento = item.departamento;
        if (item.ciudad) payload.ciudad = item.ciudad;
        if (item.direccion) payload.direccion = item.direccion;

        const clinica = await createClinica(payload);
        resultados.creados++;
        resultados.detalle_creados.push(clinica);
      } catch (err) {
        resultados.errores++;
    ...
    if (resultados.detalle_creados.length > 0) {
      setRecientes((prev) => [
        ...resultados.detalle_creados.map((c) => ({ id: c.id, nombre: c.razon_social, telefono: c.telefono })),
        ...prev,
      ]);
    }
    showToast(`${resultados.creados} clínicas creadas`, resultados.errores > 0 ? "warning" : "success");
  ...
  return (
    <Section
      title="Carga Masiva — Clínicas"
      subtitle="Subí un archivo CSV con columnas: nombre, teléfono, contacto compras, departamento, ciudad."
    >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Text variant="bodySmBold" className="text-slate-700">
                Vista previa ({preview.length} clínicas)
              </Text>
              <div className="flex gap-2">
                ...
                >
                  {uploading ? "Subiendo..." : `Crear ${preview.length} clínicas`}
                </Button>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-slate-50">
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-left py-2 px-3 text-[11px] font-bold uppercase text-slate-400">Contacto</th>
                    <th className="w-8"></th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className={cn(
              ...
              <div>
                <Text variant="bodySmBold" className={result.errores > 0 ? "text-amber-700" : "text-emerald-700"}>
                  {result.creados} clínicas creadas{result.errores > 0 ? `, ${result.errores} con errores` : ""}
                </Text>
...
{`nombre;telefono;contacto;tel_contacto;departamento;ciudad;direccion
OdontoCenter SRL;021555444;Lic. María;0981123456;Central;Asunción;Av. España 1234
Dental Plus;021666555;Carlos;0971222333;Central;San Lorenzo;`}
            </pre>
            <Text variant="mutedXs" className="text-slate-400">
              Columnas mínimas: nombre + teléfono. Opcionales: contacto, tel_contacto, departamento, ciudad, dirección.
            </Text>
          </div>
        )}
      </div>
...
function ListaRecientes({ recientes }) {
  ...
  return (
    <Section
      title={`Cargadas (${recientes.length})`}
      subtitle="Clínicas registradas en esta sesión."
    >
      <div className="p-4">
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {recientes.map((r) => (
            ...
            >
              ...
              <button
                onClick={() => router.push(`/ventas-crm/contactos/clinicas/${r.id}`)}
...
export default function NuevoProspectoClinicaPage() {
  ...
  return (
    <div className="flex flex-col flex-1 h-screen overflow-hidden bg-slate-50/50">
      <PageHeader
        ...
        subtitle="Clínicas · Carga de prospectos"
