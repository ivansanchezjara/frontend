    ...
    <div className="flex flex-col flex-1 h-screen overflow-hidden bg-slate-50/50">
      ...
      <PageHeader
        ...
        subtitle={
          <>
            <Package size={12} />
            <span>{`${consignacion.responsable} • ${consignacion.destino}`}</span>
          </>
        }
      >
        ...
        <span
          className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest ${consignacion.estado === "APROBADO" ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : "bg-amber-100 text-amber-700 border border-amber-200"}`}
        >
          {consignacion.estado}
        </span>
      </PageHeader>
          ...
          <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden min-h-[500px]">
            ...
            <div className="p-8">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {consignacion.items?.map((item, i) => (
                        <div
                          key={i}
                          className="p-5 bg-slate-50 border border-slate-100 rounded-[28px] flex items-center justify-between group"
                        >
              ...
              {activeTab === "devoluciones" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <p className="text-center py-20 text-slate-300 font-black uppercase tracking-widest text-xs italic">
                    Registros de retornos a depósito se mostrarán aquí.
                  </p>
                </div>
              )}
              ...
              {activeTab === "liquidaciones" && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <p className="text-center py-20 text-slate-300 font-black uppercase tracking-widest text-xs italic">
                    Registros de ventas y cierres parciales se mostrarán aquí.
                  </p>
                </div>
              )}
                  ...
                  <div className="flex gap-4 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-700">
                        Comprobante generado por {consignacion.usuario_nombre}
                      </p>
            ...
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden p-8 space-y-6">
                ...
                <p className="text-slate-500 font-bold text-sm leading-relaxed">
                  Este formulario registrará los ítems que regresan físicamente
                  al stock desde la consignación del responsable.
                </p>
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex items-center gap-4">
                  <AlertCircle className="text-amber-500" />
                  <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">
                    Función en desarrollo para despliegue de ítems parciales.
                  </p>
                </div>
                <div className="flex justify-end gap-3 pt-4">
                  <button
                    onClick={() => setShowDevolucionModal(false)}
                    className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase"
                  >
                    Cerrar
                  </button>
                </div>
