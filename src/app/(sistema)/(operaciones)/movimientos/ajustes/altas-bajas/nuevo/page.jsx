...
import { getDepositos, crearAjusteRapido } from '@/services/apis/movimientos';
...
export default function NuevoAjusteRapidoPage() {
    ...
    const { execute: createAction, loading: isSubmitting } = useApi(crearAjusteRapido, { auto: false });

    useEffect(() => {
        async function loadData() {
            try {
                const [dDep, dLotes] = await Promise.all([
                    fetchDepositos(),
    ...
    const handleSubmit = async (e) => {
        ...
        const isConfirmed = await confirm(
            "¿Estás seguro de guardar este borrador de ajuste rápido?",
            "Guardar Borrador"
        );
        ...
        try {
            ...
            showToast("Borrador de ajuste rápido creado con éxito", "success");
            router.push(`/movimientos/ajustes/altas-bajas/${result.id}`);
        } catch (error) {
        ...
        <div className="flex flex-col flex-1 h-screen overflow-hidden bg-slate-50/50">
            <PageHeader
                ...
                subtitle={
                    <>
                        <Layers size={12} />
                        Completá los datos del ajuste y cargá las líneas de alta/baja.
                    </>
                }
            >
                <button
