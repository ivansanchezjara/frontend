          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {transferencias.map((transf) => (
                <div
                  key={transf.id}
                  className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm hover:shadow-lg hover:border-blue-200 transition-all group flex flex-col md:flex-row items-center gap-6"
                >
                  {/* Icono de Estado */}
                  <div
                    className={`w-16 h-16 rounded-[22px] flex items-center justify-center shrink-0 shadow-sm border ${transf.estado === "APROBADO" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-blue-50 text-blue-600 border-blue-100"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0 text-center md:text-left">
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                      ...
                      <span
                        className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${transf.estado === "APROBADO" ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}
                      >
                        {transf.estado}
                      </span>
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start gap-6 mt-4 text-slate-400 font-bold text-[10px] uppercase tracking-widest">
                      ...
                      <span className="flex items-center gap-2">
                        <User size={14} /> {transf.usuario_nombre}
                      </span>
                    </div>
                  </div>
